...
Page({
  ...
  onLoad(options) {
    let that = this;
    app.initShare();
    let sessionName = app.globalData.sessionname;
    let tempDate = '第'+app.globalData.sessionWeek + '周第' + (app.globalData.sessionNo+1) + '课';
    this.oToast = this.selectComponent("#toast"); 
    this.oAuth = this.selectComponent("#auth");
    this.setData({user:wx.getStorageSync('user'),sessionName:sessionName,qSessionTime:tempDate})
    app.api.post({url:uGetStatistic,data:{cslId:app.globalData.cslId
    }}).then(res => {
